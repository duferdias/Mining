...
# Com o script "sproxy-dgb" é possível instalar, configurar e executar
# o stratum proxy com o arquivo de configuração stratum-proxy-yescrypt
# para receber mineradores de DigiByte e redirecioná-los para a zpool.ca.
...
# [ SCRIPT ]
# Obtém a carteira de DigiByte
if [ -z "${carteira-}" ]; then
  echo
  read -p "Entre com a carteira de DigiByte: " carteira
fi
...
# Insere as variáveis
sed -i 's/XMY/DGB/g' stratum-proxy-yescrypt.conf

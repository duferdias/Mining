...
# o stratum proxy com o arquivo de configuração stratum-proxy-yespowerr16
# para receber mineradores de YENTEN e redirecioná-los para yenten.luckypool.org.
...
wget https://github.com/duferdias/stratum-proxy/releases/download/v0.8.1.1/stratum-proxy-yespowerr16.conf -O stratum-proxy-yespowerr16.conf

# Convert format dos2unix
# awk '{printf "%s\r\n", $0}' stratum-proxy-yespowerr16.conf
# Insere as variáveis
sed -i "s/\"user\" :.*/\"user\" \: \"${WLogin_WName}\"\,/" stratum-proxy-yespowerr16.conf
sed -i "s/\"password\" :.*/\"password\" \: \"${passwordMiner}\"\,/" stratum-proxy-yespowerr16.conf
sed -i "s/\"apiUser\":.*/\"apiUser\"\: \"${usuario}\"\,/" stratum-proxy-yespowerr16.conf
sed -i "s/\"apiPassword\":.*/\"apiPassword\"\: \"${senha}\"\,/" stratum-proxy-yespowerr16.conf
...
sleep 3
sudo screen -dmS sproxy java -jar stratum-proxy-0.8.1-malthraxcrypto.jar --conf-file=stratum-proxy-yespowerr16.conf
sleep 2
clear

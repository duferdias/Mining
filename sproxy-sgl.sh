...
# Com o script "sproxy-sgl" é possível instalar, configurar e executar
# o stratum proxy com o arquivo de configuração stratum-proxy-neoscrypt
# para receber mineradores de SIGIL e redirecioná-los para sigilmining.com.
...
# [ SCRIPT ]
# Obtém o Nome e a ID do usuário minerador de Sigil na forma: <WorkerLogin.WorkerName>
if [ -z "${WLogin_WName-}" ]; then
  echo
  read -p "Entre com o Nome e a ID do usuário minerador de SGL na forma <WorkerLogin.WorkerName>: " WLogin_WName
fi
# Obtém a senha (password) do usuário minerador de Sigil definida na Pool
...
wget https://github.com/duferdias/stratum-proxy/releases/download/v0.8.1.1/stratum-proxy-neoscrypt.conf -O stratum-proxy-neoscrypt.conf

# Convert format dos2unix
# awk '{printf "%s\r\n", $0}' stratum-proxy-neoscrypt.conf
# Insere as variáveis
sed -i "s/\"user\" :.*/\"user\" \: \"${WLogin_WName}\"\,/" stratum-proxy-neoscrypt.conf
sed -i "s/\"password\" :.*/\"password\" \: \"${passwordMiner}\"\,/" stratum-proxy-neoscrypt.conf
sed -i "s/\"apiUser\":.*/\"apiUser\"\: \"${usuario}\"\,/" stratum-proxy-neoscrypt.conf
sed -i "s/\"apiPassword\":.*/\"apiPassword\"\: \"${senha}\"\,/" stratum-proxy-neoscrypt.conf
...
sleep 3
sudo screen -dmS sproxy java -jar stratum-proxy-0.8.1-malthraxcrypto.jar --conf-file=stratum-proxy-neoscrypt.conf
sleep 2
clear

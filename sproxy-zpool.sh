...
# Com o script "sproxy-zpool" é possível instalar, configurar e executar
# o stratum proxy com o arquivo de configuração stratum-proxy-yescrypt
# para receber mineradores de criptomoedas, cuja mineração seja compatível
# com o algorítimo yescrypt e redirecioná-los para zpool.ca. Uma vez que
# na zpool.ca não se minera diretamente a criptomoeda escolhida, mas sim 
# três outas criptomoedas de forma alternada, verifique antes de iniciar
# a mineração se a zpool.ca oferece o pagamento. O Bitcoin é a principal 
# criptomoeda que a zpool.ca paga pela mineração com yescrypt das 3 outras 
...
wget https://github.com/duferdias/stratum-proxy/releases/download/v0.8.1.1/stratum-proxy-yescrypt.conf -O stratum-proxy-yescrypt.conf

# Convert format dos2unix
# awk '{printf "%s\r\n", $0}' stratum-proxy-yescrypt.conf
# Insere as variáveis
sed -i "s/\"user\" :.*/\"user\" \: \"${carteira}\"\,/" stratum-proxy-yescrypt.conf
sed -i "s/\"password\" :.*/\"password\" \: \"${simbolo}\"\,/" stratum-proxy-yescrypt.conf
